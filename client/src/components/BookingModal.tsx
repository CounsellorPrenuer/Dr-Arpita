...
export default function BookingModal({ open, onOpenChange, planId, title, category, price }: Props) {
  ...
  const openMailDraft = () => {
    const subject = encodeURIComponent(`Skillzy booking enquiry: ${title}`);
      ...
      <DialogContent className="sm:max-w-lg max-h-[92vh] overflow-y-auto">
        ...
        <div className="grid gap-4">
          ...
          <div>
            ...
            {couponMessage && <p className="text-sm mt-2 flex gap-2 items-start"><CheckCircle2 className="w-4 h-4 mt-0.5 text-emerald-500" />{couponMessage}</p>}
          </div>
        </div>
        <div className="grid sm:grid-cols-2 gap-3 pt-2">
          <Button variant="outline" onClick={openMailDraft}><Mail className="w-4 h-4 mr-2" />Email Instead</Button>
          <Button onClick={pay} disabled={isProcessing} className="bg-gradient-to-r from-blue-500 to-violet-500">
