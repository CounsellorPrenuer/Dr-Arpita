  ...
  return (
    <section className="mt-24">
      <div className="text-center max-w-4xl mx-auto mb-12 px-4">
        <h2 className="font-heading font-black text-3xl sm:text-4xl lg:text-5xl text-white mb-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 max-w-7xl mx-auto px-2 sm:px-4">
        {plans.map((plan) => (
          <Card key={plan.planId} className="h-full flex flex-col overflow-hidden bg-slate-800/50 border-slate-700 p-0">
            ...
            <div className="p-6 flex flex-col flex-1">
              <h3 className="font-heading font-bold text-xl text-white mb-3 text-center">{plan.title}</h3>
              <div className="text-3xl font-bold text-blue-400 mb-4 text-center">{formatCurrency(plan.price)}</div>
              <p className="text-sm text-slate-400 leading-relaxed flex-1 mb-6">{plan.description}</p>
              <Button className="w-full bg-gradient-to-r from-blue-500 to-violet-500 mt-auto" onClick={() => onBuyClick(plan)}>
                Buy Now
              </Button>
            </div>
          </Card>
